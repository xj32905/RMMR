...
###W4
1，真实相机的接入。
问题:①识以为识别出来的节点为电脑本身节点。
     ②MVS与ROS不能同时打开
2，rqt图像读取。
问题：刚开始没有安装对应的包。
常见命令（按筛选流程）：
①red-mask：刚开始噪点明显，调高S（50）至（70），V（50）至（70）。
②调了这两个后想到，既然己经筛出了长条，只需告诉机器使其进行长宽比粗暴筛选，(噪点并不会是特别长条的形状)
③最后运行。

以上方法只起到了简单筛选，对更复杂环境测试未知。
此代码先开始运用另一套详细筛选机制，但由于筛选条件严格，故难成框。所以result中会看出近似两个条件下的结果？
