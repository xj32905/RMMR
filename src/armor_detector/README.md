...
| 缩放方式 | 等比缩放 + 黑底填充 | 数字长边等比缩到 24，再居中置于 32×32 黑色画布（保留黑边，匹配训练分布） |
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `roi_debug_save` | false | 保存 ROI 与 `32×32` 输入图 |
...
| `vertical_bias` | 0.25 | ROI 垂直偏移（h 的倍数），正值=下移，[-0.1, 0.3] 均稳定 |
...
**关键发现：**
- 压缩后的输出视频破坏了数字细节，原始输入视频可正常分类
- `vertical_bias` 在 [-0.1, 0.3] 均稳定（51-52%），方案鲁棒
...
4. **ONNX 分类准确率已大幅提升**：居中装甲平面 ROI 方案将 `three` 命中率从 11% 提升至 52%，最高置信度 0.97。剩余 48% 未命中帧主要为装甲板角度不佳导致数字不可见。
5. **真实多装甲板场景待验证**：贪心配对已稳定输出，但实机多目标画面仍需验证。
...
- ROI 裁剪：基于灯条中心间距和均长的居中矩形（`getArmorRectFromBars` + `cropDigitRoi`），宽 1.0x，高 1.0x，`vertical_bias=0.25`
- 灯条配对：贪心匹配，每灯条至多参与一次，按综合评分排序
- 时间滤波：5 帧历史，2 票多数，仅正向覆盖（不反向压制正确分类）
- 低置信度抑制：非 `not_armor` 且置信度 <0.45 → 压制
- ONNX 数字识别全链路跑通，默认启用
